...
class TreasureWinCheck(WinCheck):
    ...
    def search_for_winners(self, info: DungeonCrawlGameInfo) -> List[str]:
        winning_bots_names = [bot.name for bot in info.players if sum([
            worth for worth in [get_dungeoneer_treasure_worth(d) for d in bot.info.items if isinstance(d, Dungeoneer)]
